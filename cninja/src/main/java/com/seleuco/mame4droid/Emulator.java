	...
	public static void emulate(final String libPath,final String resPath){
		...
		Thread t = new Thread(new Runnable(){
			...
			public void run() {
				...
				{
					Log.d(TAG, "Before parse URI");


					final String custom_rom_filename = "cninja.zip";
//					final String custom_rom_file_fullname = 	mm.getFilesDir().getAbsolutePath()+ "/" + custom_rom_filename;
					final String custom_rom_file_fullname = 	mm.getFilesDir().getAbsolutePath()+ "/" + custom_rom_filename;
					InputStream in = mm.getResources().openRawResource(R.raw.cninja);
					File file = new File(custom_rom_file_fullname);

					try
					{
						Log.d(TAG, "tony check cninja asset hash : " + in.available());
						Log.d(TAG, "tony check cninja installed rom file hash : " + file.length());

						if (file.exists() == true && in.available() != file.length() ){
							Log.d(TAG, custom_rom_file_fullname + " another version already exists - delete");
							file.delete();
						}
					} catch (IOException e){
					...
					if(!file.exists()) {
						...
						Log.d(TAG, "custom_rom_file_path  = " +custom_rom_file_fullname);

							OutputStream out = new FileOutputStream(custom_rom_file_fullname);
							int read = 0;

							while ((read = in.read(buffer))!= -1) {
								Log.d(TAG, "Extracting rom file to" + custom_rom_file_fullname);
					...
					java.io.File f = new java.io.File(custom_rom_file_fullname);
					final String name = f.getName();
					String path = f.getAbsolutePath().substring(0, f.getAbsolutePath().lastIndexOf(File.separator));
					Log.d(TAG, "Filename=" + name + "  Path=" + path);
					Log.d(TAG, "Filename=" + name + "  Path=" + path);
//					Log.d(TAG, "Filename=" + name + "  Path=" + path);
//					Log.d(TAG, "Filename=" + name + "  Path=" + path);
...
//					Emulator.setValueStr(Emulator.ROM_NAME, "tektagt.zip");
//					Emulator.setValueStr(Emulator.ROM_PATH, "/storage/emulated/0/MAME4droid/roms/");
					//getSharedPreferences().getBoolean(PREF_GLOBAL_SHOW_INFOWARNINGS,true);

					//sunghook remove disclaimer & game info
					//--> find userpreferences.xml  , setting the key "PREF_GLOBAL_SHOW_INFOWARNINGS" will turn off the disclaimer!
					extROM = true;
